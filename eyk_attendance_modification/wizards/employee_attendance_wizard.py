...
class EmployeAttendanceWizard(models.TransientModel):
    ...
    def action_print(self):
        ...
        [data] = self.read()
        from_month_name = self.date_from.strftime("%B")
        to_month_name = self.date_to.strftime("%B")
        from_yr_name = self.date_from.strftime("%Y")
        to_yr_name = self.date_to.strftime("%Y")
        print(from_month_name, to_month_name, from_yr_name, to_yr_name)
        if from_month_name != to_month_name or from_yr_name != to_yr_name:
            raise ValidationError(_("Month/Year should be same in 'from' and 'to' fields to generate monthly report."))
